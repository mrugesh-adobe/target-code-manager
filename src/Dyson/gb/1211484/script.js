    ...
    var e = function (t, e, a, n) {
      ...
      var c = function (t) {
        if (t.animationName === i && "object" == typeof t.target) {
          ...
          console &&
            console.info &&
            console.info("Node " + e + " ready! Execute: " + s, t.target),
            "function" == typeof a &&
              s &&
  ...
  success: function (el) {
    el.addEventListener("click", function () {
      ...
      var selectedEls = document.querySelectorAll(
        "[data-testid='question-grid'] > div[aria-checked='true'] h3"
      );
      var selectedTexts = Array.from(selectedEls).map((el) =>
        el.innerText.trim()
      );
      ...
      // Add tool name
      var pageTitle =
        window?.dataLayer?.pageInfo?.pageTitle
          ?.split(" | ")?.[1]
          ?.split(" ")
          ?.join("-") ?? "";
...
// bestFor profile parameter for PLP pages
Flickerlessly.onReady({
  selector: "#productListingFacet > div > ul > li:nth-child(2)",
  persist: true,
  success: function (el) {
    el.addEventListener("click", function (event) {
      // Check if clicked element is a checkbox
      if (event.target.type === "checkbox") {
        // Get all checked checkboxes in the bestFor filter section within this element
        var checkedBoxes = el.querySelectorAll(
          "input[type='checkbox']:checked"
        );
        
        // Extract text values from checked checkboxes
        var selectedTexts = Array.from(checkedBoxes).map(function(checkbox) {
          // Try to get text from associated label or parent element
          var label = checkbox.closest('li').querySelector('label');
          if (label) {
            return label.innerText.trim();
          }
          // Fallback: get text from parent li element
          var parentLi = checkbox.closest('li');
          if (parentLi) {
            // Clone the element to avoid modifying original
            var clone = parentLi.cloneNode(true);
            // Remove the checkbox input to get only text
          ...
          return text.length > 0; // Remove empty strings
        });

        // Create pipe-separated string
        var bestForValue = selectedTexts.join("|");
        
        console.log("bestFor selections:", bestForValue);

        // Prepare profile parameters
        var targetParams = {
          "profile.bestFor": bestForValue
        };
        ...
        adobe.target.getOffer({
          ...
          error: function (status, error) {
            console.error("Target error for bestFor:", status, error);
          },
        });
      }
    });
